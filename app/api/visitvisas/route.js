...
export async function GET() {
  try {
    const visitvisas = db.prepare("SELECT * FROM visitvisas").all();
    return NextResponse.json(visitvisas);
  } catch (err) {
...
export async function POST(req) {
  try {
    ...
    const insertedVisa = db.prepare("SELECT * FROM visitvisas WHERE id = ?").get(result.lastInsertRowid);
...
export async function PUT(req) {
  try {
    ...
    const updatedVisa = db.prepare("SELECT * FROM visitvisas WHERE id = ?").get(visa.id);
...
export async function DELETE(req) {
  try {
    ...
    const stmt = db.prepare("DELETE FROM visitvisas WHERE id = ?");

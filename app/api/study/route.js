...
export async function GET() {
  try {
    const study = db.prepare("SELECT * FROM study").all();
...
export async function POST(req) {
  try {
    ...
    const insertedVisa = db.prepare("SELECT * FROM study WHERE id = ?").get(result.lastInsertRowid);
...
export async function PUT(req) {
  try {
    ...
    const updatedVisa = db.prepare("SELECT * FROM study WHERE id = ?").get(visa.id);
...
export async function DELETE(req) {
  try {
    ...
    const stmt = db.prepare("DELETE FROM study WHERE id = ?");

...
if (!columnExists("services", "addonHeading")) {
  db.prepare(`ALTER TABLE services ADD COLUMN addonHeading TEXT`).run();
}
if (!columnExists("services", "addonDescription")) {
  db.prepare(`ALTER TABLE services ADD COLUMN addonDescription TEXT`).run();
}
